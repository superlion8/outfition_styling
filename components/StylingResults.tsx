...
export const StylingResults: React.FC<StylingResultsProps> = ({ items, onRegenerate, outfitCount }) => {
  ...
  // Initialize outfits when count changes
  useEffect(() => {
    // Only initialize if the length is different to preserve state during other updates
    setOutfits(prev => {
      // If we already have outfits and the count matches the initial request (or we have modified it), 
      // we might want to be careful. But strictly sticking to the requested count on mount/change is safer for "Regenerate".
      // However, if the user deleted items, prev.length < outfitCount. We probably don't want to re-add them instantly unless outfitCount actually changed.
      
      // Simple logic: Reset/Init if the target count is different from current length AND it seems like a reconfiguration.
      // Ideally, we'd want to just init once.
      // For now, let's just initialize if empty or if count explicitly changed from parent (re-styling).
      if (prev.length === 0 || prev.length !== outfitCount) {
         // This condition is tricky if we delete items. 
         // Let's assume if the prop changes, we reset. 
         // But deleting changes local length, not prop.
         // We will just return prev if the length is > 0 and <= outfitCount to avoid overwriting deletions?
         // Actually, let's just use the prop to seed.
      }
      
      return Array.from({ length: outfitCount }).map((_, i) => ({
