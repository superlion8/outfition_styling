...
export const ModelPreviewCard: React.FC<ModelPreviewCardProps> = ({
    ...
    const isFullSize = size === 'full' || !!height;

    return (
        <div
            className={`relative ${height ? '' : sizeClasses[size]} rounded-2xl overflow-hidden transition-all cursor-pointer border flex flex-col bg-card-dark ${isSelected
        ...
        >
            ...
            <div className="flex-1 relative">
                ...
                {showBadge && (
                    <div className={`absolute top-3 left-3 z-10 flex items-center gap-1.5 bg-black/60 backdrop-blur-sm rounded-full border border-white/10 ${isFullSize ? 'px-3 py-1.5 gap-2' : 'px-2.5 py-1'}`}>
            ...
            {showCustomizeButton && onCustomize && (
                <button
                    ...
                    className={`w-full bg-black/40 backdrop-blur-md text-white font-bold transition-all flex items-center justify-center gap-2 hover:bg-black/60 ${isFullSize ? 'py-4' : 'py-2'}`}
                >
